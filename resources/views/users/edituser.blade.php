                    ...
                    <form action="{{ route('users.update', $user->id) }}" method="POST">
                        ...
                        <div class="mb-4">
                            <label class="relative flex">
                                <input class="form-input peer w-full rounded-lg bg-slate-150 px-3 py-2 pl-9 ring-primary/50 placeholder:text-slate-400 hover:bg-slate-200 focus:ring dark:bg-navy-900/90 dark:ring-accent/50 dark:placeholder:text-navy-300 dark:hover:bg-navy-900 dark:focus:bg-navy-900"
                        ...
                        <div class="mb-4">
                            <label class="relative flex">
                                <select class="form-select peer w-full rounded-lg bg-slate-150 px-3 py-2 pl-9 ring-primary/50 placeholder:text-slate-400 hover:bg-slate-200 focus:ring dark:bg-navy-900/90 dark:ring-accent/50 dark:placeholder:text-navy-300 dark:hover:bg-navy-900 dark:focus:bg-navy-900"
                                    ...
                                    <option value="property_leasion_agent" {{ $user->access_level === 'property_leasion_agent' ? 'selected' : '' }}>Property Leasion Agent</option>
                                    <option value="property_manager" {{ $user->access_level === 'property_manager' ? 'selected' : '' }}>Property Manager</option>
                                    <option value="property_owner" {{ $user->access_level === 'property_owner' ? 'selected' : '' }}>Property Owner</option>
                                    <option value="parking_inspector" {{ $user->access_level === 'parking_inspector' ? 'selected' : '' }}>Parking Inspector</option>
                                    <option value="company_administrator" {{ $user->access_level === 'company_administrator' ? 'selected' : '' }}>Company Administrator</option>
                                </select>
                            </label>
                            @error('access_level')
                            <span class="text-tiny+ text-error">{{ $message }}</span>
                            ...
                            <label class="relative flex">
                                <select class="form-select peer w-full rounded-lg bg-slate-150 px-3 py-2 pl-9 ring-primary/50 placeholder:text-slate-400 hover:bg-slate-200 focus:ring dark:bg-navy-900/90 dark:ring-accent/50 dark:placeholder:text-navy-300 dark:hover:bg-navy-900 dark:focus:bg-navy-900"
                                    ...
                                    @foreach ($addresses as $address)
                                        <option value="{{ $address }}" {{ $user->property === $address ? 'selected' : '' }}>{{ $address }}</option>
                                    @endforeach
                                </select>
                            </label>
                            @error('property')
